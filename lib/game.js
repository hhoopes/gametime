...
let game = class Game {
  ...
  instructions(){
    ...
    this.ctx.fillText("Green", 35, 60);
    this.ctx.fillText("Yellow", this.canvas.width - 118, 60);
    this.ctx.fillText("Red", 35, this.canvas.height - 35);
    this.ctx.fillText("Blue", this.canvas.width - 100, this.canvas.height - 35);
    this.ctx.textAlign = "center";
    this.ctx.fillText("<    Choose a color    >", this.canvas.width/2, 60);
    this.ctx.fillText("<    Find your controller    >", this.canvas.width/2 - 10, this.canvas.height - 35);
    this.ctx.fillText("Touch START when ready", this.canvas.width/2, this.canvas.height/2);
  }
  ...
  beginRound(){
    ...
    requestAnimationFrame(function gameTime(){
      ...
      if (game.finished()) {
        ...
        game.ctx.strokeText("Touch START to restart", game.canvas.width/2, game.canvas.height/2 + 60);
        game.ctx.fillText("Touch START to restart", game.canvas.width/2, game.canvas.height/2 + 60);
