...
let game = class Game {
  ...
  beginRound(){
    ...
    requestAnimationFrame(function gameTime(){
      ...
      game.balls.forEach(function(ball){ ball.draw(game.ctx).move(game.quadrants, game.paddles, game.players, game.playerSize*game.blockSize, game.canvas); });
  ...
  createGrid(){
    ...
    this.players.push(new Player(1, 0, 0, playerSpace, canvasSize));
    this.players.push(new Player(2, canvasSize - playerSpace, 0, playerSpace, canvasSize));
    this.players.push(new Player(3, 0, canvasSize - playerSpace, playerSpace, canvasSize));
    this.players.push(new Player(4, canvasSize - playerSpace, canvasSize - playerSpace, playerSpace, canvasSize));

    for (let y = 0; y < numBlocks; y++) {
      for (let x = 0; x < numBlocks; x++) {
        // Top Left
        if (x*blockSize >= playerSpace || y*blockSize >= playerSpace){
          this.quadrants[0].push(new Block(x*blockSize, y*blockSize, blockSize, '#04395E'))
        }
        // Top Right
        if (((x+1)*blockSize + deadSpace) <= canvasSize - playerSpace || y*blockSize >= playerSpace){
          this.quadrants[1].push(new Block(deadSpace+(x*blockSize), y*blockSize, blockSize, '#70A288'))
        }
        // Bottom Left
        if (x*blockSize >= playerSpace || ((y+1)*blockSize + deadSpace) <= canvasSize - playerSpace){
          this.quadrants[2].push(new Block(x*blockSize, deadSpace+(y*blockSize), blockSize, '#DAB785'))
        }
        // Bottom Right
        if (((x+1)*blockSize + deadSpace) <= canvasSize - playerSpace || ((y+1)*blockSize + deadSpace) <= canvasSize - playerSpace){
          this.quadrants[3].push(new Block(deadSpace+(x*blockSize), deadSpace+(y*blockSize), blockSize, '#D5896F'))
        }
      }
    }
  }
